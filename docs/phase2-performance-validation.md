...
配套脚本为 [`tools/perf/phase2-pktmon.ps1`](../tools/perf/phase2-pktmon.ps1) 和 [`tools/perf/analyze-presentmon.ps1`](../tools/perf/analyze-presentmon.ps1)。前者只包装 Windows 自带的 `pktmon`，不会删除全局过滤器，也不会替用户启动或停止 Minecraft 服务端；后者只分析单一 Minecraft PID/SwapChain 的逐帧 CSV。
...
- A：`Entities.Item.Options.VisibilityRateLimit.BucketSize: 4096`、`RestorePerTick: 4096`，作为等效不限流。
...
### 5.3 无管理员权限的服务器出站证据：JFR SocketWrite
...
### 5.4 无管理员权限的客户端证据：原版 F3+L profile
...
`pktmon counters --json` 用于快速确认包计数和丢包；精确 bytes 与时间分桶以 pcapng 分析为准。当前机器没有 tshark/capinfos 时，保留 ETL、pcapng 和 counters JSON 作为原始证据，不能从 counters 猜测字节数。
...
至少在 Paper/client 26.1.2 上完整执行，并在支持目标包含 26.2 时复测关键路径：
...
- `IV_PERF` JSONL。
- pktmon RunId claim、capture manifest（异常时保留 starting/recovery 状态，正常时为 completed）、ETL、counters JSON、stats TXT、pcapng 和 component ID。
- 服务端日志、错误数、PresentMon 版本与二进制 SHA-256、客户端逐帧 CSV、分析 JSON、兼容性勾选结果。

缺少上述原始证据的结果只能作为线索，不能作为合并或默认启用优化的依据。
