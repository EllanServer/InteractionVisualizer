...
public class V1_17 extends NMSWrapper {
    ...
    public V1_17() {
        try {
            ...
            dataWatcherCustomNameField = net.minecraft.world.entity.Entity.class.getDeclaredField("aJ");
            dataWatcherCustomNameVisibleField = net.minecraft.world.entity.Entity.class.getDeclaredField("aK");
            dataWatcherSilentField = net.minecraft.world.entity.Entity.class.getDeclaredField("aL");
            dataWatcherNoGravityField = net.minecraft.world.entity.Entity.class.getDeclaredField("aM");
    ...
    public List<BoundingBox> getBoundingBoxes(BlockPosition pos) {
        ...
        return shape.toList().stream().map(each -> new BoundingBox(each.a + pos.getX(), each.b + pos.getY(), each.c + pos.getZ(), each.d + pos.getX(), each.e + pos.getY(), each.f + pos.getZ())).collect(Collectors.toList());
    }
    ...
    public String getBannerCustomName(Block block) {
        ...
        net.minecraft.world.level.block.entity.TileEntity tileEntity = worldServer.getTileEntity(new net.minecraft.core.BlockPosition(block.getX(), block.getY(), block.getZ()));
        if (tileEntity == null) {
            return "";
        }
        return tileEntity.Z_().getString("CustomName");
    }

    @Override
    public WrappedIterable<?, Entity> getEntities(World world) {
        WorldServer worldServer = ((CraftWorld) world).getHandle();
        return new WrappedIterable<>(worldServer.G.d().a(), net.minecraft.world.entity.Entity::getBukkitEntity);
    }
    ...
    public void spawnArmorStand(Collection<Player> players, ArmorStand entity) {
        try {
            ...
            spawnEntityLivingPacketFields[2].setInt(packet1, IRegistry.Y.getId(type));
